...
//  Created by Ravi Dwivedi on 2023.
//
...
public struct MarkDownAttributedStringParser: MarkupVisitor {
    ...
    mutating func parserResult(from document:Document) ->[ParserResult]{
        
        var results = [ParserResult]()
        var currentAttributedString = NSMutableAttributedString()
        func appendCurrentAttributedString(){
            if !currentAttributedString.string.isEmpty {
                let currentAttrStringToAppend = (try? AttributedString(currentAttributedString,including: \.uiKit)) ?? AttributedString(stringLiteral: currentAttributedString.string)
        ...
        document.children.forEach { markup in
          let attrString = visit(markup)
            ...
            if let codeBlock = markup as? CodeBlock {
                ...
                currentAttributedString.append(attrString)
            }
    ...
    mutating public func visitOrderedList(_ orderedList: OrderedList) -> NSAttributedString {
        ...
        if orderedList.hasSuccessor {
            result.append(orderedList.isContainedInList ? .singleNewline(withFontSize: newLineFontSize) : .doubleNewline(withFontSize: baseFontSize))
        }
    ...
    mutating public func visitBlockQuote(_ blockQuote: BlockQuote) -> NSAttributedString {
        ...
        for child in blockQuote.children {
            ...
            quoteAttributedString.addAttribute(.foregroundColor, value: UIColor.systemPink)
